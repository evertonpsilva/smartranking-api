...
export class PartidasService {
    ...
    async atribuirDesafioPartida(idDesafio:string, atribuirDesafioPartidaDto: AtribuirDesafioPartidaDto): Promise<Partida>{

        const {def, resultado} = atribuirDesafioPartidaDto;

        const desafioCadastrado = await this.desafiosService.buscarDesafioPorId(idDesafio);

        if(!desafioCadastrado.jogadores.includes(def)){
            throw new BadRequestException("Jogador vitorioso informado não pertece a este desafio");
        }
        ...
        this.desafiosService.finalizarDesafio(desafioCadastrado._id, {status: DesafioStatus.REALIZADO, partida: partida});
