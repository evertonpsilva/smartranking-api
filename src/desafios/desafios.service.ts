...
export class DesafiosService {
    ...
    async criarDesafio(criarDesafioDto: CriarDesafioDto): Promise<Desafio>{
        ...
        if(!idsJogadores.includes(solicitante._id)) throw new BadRequestException("O solicitante deve ser também um jogador");
         
        await this.jogadoresService.consultarJogadoresPorIds(idsJogadores);

        const categoriaDesafio = await this.categoriasService.buscarCategoriaDoJogador(solicitante._id);
    ...
    async consultarDesafios(idJogador: string): Promise<Desafio[]>{
        ...
        if(idJogador) return await this.desafioModel.find().populate('jogadores').where('jogadores').in([idJogador]).exec();
